...
# ---------------------------------------------------------------------------------
        ...
        if self.decode_model is not None and self.use_pretrained_models == True:
            ...
            # self.mp_trainer_multihead = MixedPrecisionTrainer(
            #     model=self.multihead,
            #     use_fp16=False,
            #     fp16_scale_growth=fp16_scale_growth,
            #     clip_norm=clip_norm,
            # )
# ------------------------------------------------------------------------------------------
        if FusedAdam is not None and use_fused_adam:
            ...
            # self.opt_multihead = AdamW(
            #     self.mp_trainer_multihead.master_params, lr=self.lr,
            #     weight_decay=self.weight_decay, betas=betas
            # )
        ...
        #这里还不能加，因为预训练模型的resnet没有定义过convert_to_fp16这个函数；
        # if self.use_pretrained_models:
        #     self.mp_trainers["mp_resnet_trainer"] = MixedPrecisionTrainer(
        #         model=self.pre_trained_models["resnet"],
        #         use_fp16=self.use_fp16,
        #         fp16_scale_growth=fp16_scale_growth,
        #         clip_norm=clip_norm,
                ...
                if dist.get_rank() != n:
                    ...
                    while (
                        ...
                        if self.use_pretrained_models:
                            ...
                            outputs = resnet(batch) if self.pre_trained_models['FE'] is None else resnet(**inputs).logits
...
# ------------------------------------------------------------------------------------------------------
    def run_step(self, batch,cond):
        ...
        if self.mp_trainer_decoder is not None and self.decode_model is not None:
    ...
    def forward_backward(self, batch,cond):
        ...
        for i in range(0, batch.shape[0], self.microbatch):
            ...
            if self.use_pretrained_models:
                ...
                inputs = image_processor(text=["a dog"],images = self.origin_data_before_split,return_tensors="pt", padding=True)
            ...
            _lambda = 0.3
            loss = (losses["loss"] * weights).mean() + _lambda * losses["ssim_loss"] + (1-_lambda)*losses["L1_loss"]
