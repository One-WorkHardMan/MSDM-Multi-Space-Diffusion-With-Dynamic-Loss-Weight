    ...
    def training_losses(self, model,
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            if decode_model is not None and iteration % 10 == 0:
                ...
                #采样再输出
                z = torch.randn([model_output.shape[0], model_output.shape[1], model_output.shape[2], model_output.shape[3]],
                                device="cuda")
                ...
                with torch.no_grad():
                    ...
                    dist.barrier()
                    # pdb.set_trace()
                    Total_samples_data = [torch.zeros_like(model_output_samplers_by_edm) for i in range(dist.get_world_size())]
                    ...
                    decode_data = decode_model(Total_model_output_data,split_space) # Total_model_output_data是list类型，在Decoder代码里面其实这个 multispace并没有起作用；
                    origin_encode_data = decode_model(origin_encode_data,split_space)

                    self.generate_train_images(decode_data,origin_data_without_trans,origin_encode_data,generate_image_size = generate_image_size)

                    logger.logkv("ssim_value",self.calc_ssim_score(origin_data_without_trans, decode_data,generate_image_size))
                    logger.logkv("L1loss_value", self.L1_loss(decode_data, origin_data_without_trans,generate_image_size))
