...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x, **kwargs):
        ...
        # print("PatchEmbed-242-line-x.shape",x.shape)
        # print("PatchEmbed-243",self.patch_size,self.num_patches,self.img_size)
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224,
                 patch_size=16,
                 in_chans=3,
                 num_classes=10 , #类别数量,这个参数只在 用 类别去引导 图像的时候才奏效。
                 embed_dim=768,
                 depth=12,
                 num_heads=12,
                 mlp_ratio=4.,
                 ...
                 use_mean_pooling=True, init_scale=0.001, use_conv_last=False, num_steps=1000, #时间步，可以不是1000

                 learn_sigma=False, use_fp16=True, drop_label_prob=0.0,
        ...
        self.use_pretrained_models = use_pretrained_models #是否使用预训练模型Clip
...
#------------------------------------------------------------------------------------
        ...
        if num_classes > 0:
            ...
            self.clip_outputs_embedding = nn.Linear(512,embed_dim)
...
# ------------------------------------------------------------------------------------
    ...
    def forward_features(self, x, timesteps, y=None, force_drop_ids=None):
        ...
        # print("x:", x.shape)
        # print("self.pos_emb",self.pos_embed.shape)

        # pdb.set_trace()

        if y is not None:
            use_dropout = self.drop_label_prob > 0 and self.training
            if (use_dropout or (force_drop_ids is not None)) and self.use_pretrained_models == False:
                y = self.token_drop(y, force_drop_ids)

            if len(y.shape) == 1 :
                cls_tokens = self.class_embedding(y).unsqueeze(1)
            else:
                y = y.to(torch.float16) #模型参数已经被压缩到fp16了 但是原本这个y还是32，所以报错
                # pdb.set_trace()
                cls_tokens = self.clip_outputs_embedding(y)
                cls_tokens = cls_tokens[:, None, :] # BS 1 768

            x = torch.cat((time_tokens, cls_tokens, x), dim=1)
        else:
            x = torch.cat((time_tokens, x), dim=1)
            print(x.shape,"No Condition Y")
...
def vit_base_patch4_64(pretrained=False, **kwargs):
    ...
    model = VisionTransformer(
        img_size=64,
        patch_size=4, embed_dim=768, num_heads=16, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **new_kwargs)
    model.default_cfg = _cfg()
    return model
